...
<body class="home-body">
   ...
   <!-- NAVBAR ADMIN -->
            ...
            <table class="transactions-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <span class="payment-badge <?php echo $trx['payment_method']; ?>">
                                <?php echo strtoupper($trx['payment_method']); ?>
                            </span>
                        </td>
                        <td>
                            <span class="status-badge-trx <?php echo $trx['status']; ?>">
    ...
    <!-- Image Preview Container (Inline Style Only) -->
    ...
    document.getElementById('update_status').value = trx.status;

    const hasProof = trx.proof_path && trx.proof_path.trim() !== '' && trx.payment_method === 'transfer';

    // ✅ FIX PATH GAMBAR (MENANGANI SEMUA FORMAT PATH DATABASE)
    let finalProofPath = '';

    if (hasProof) {

        // jika database simpan: asset/proofs/file.jpg
        ...
        // jika database hanya simpan: file.jpg
        else {
        ...
        <div style="background: #f8f8f8; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                <div>
                    ...
                    <p><strong>Status:</strong> <span class="status-badge-trx ${trx.status}">${trx.status.toUpperCase()}</span></p>
                </div>
                <div>
                    <p><strong>Payment:</strong> ${trx.payment_method.toUpperCase()}</p>
                    <p><strong>Total:</strong> Rp ${parseInt(trx.total_amount).toLocaleString('id-ID')}</p>
                </div>
            </div>
        </div>
    ...
    document.getElementById('viewTransactionModal').style.display = 'block';
        ...
        // === FUNGSI PREVIEW GAMBAR (INLINE STYLE) ===
        ...
        // Event listener untuk tombol close (X) preview
        document.getElementById('previewClose').addEventListener('click', closeImagePreview);
        
        // Tutup preview jika klik di area gelap
